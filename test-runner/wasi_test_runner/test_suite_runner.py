...
class Manifest(NamedTuple):
    """Manifest generated by tools/generate_manifest.py."""
    version: int
...
def _load_config_from_manifest(test_entry: Dict[str, Any], base: Path) -> Config:
    """Load test config from a manifest entry's config path."""
    config_path = test_entry.get("config")
    if not config_path:
        return Config()
    full_path = str(base / config_path)
    if not os.path.isfile(full_path):
        return Config()
    config = Config.from_file(full_path)
    TestCaseValidator(config, full_path).validate()
    return config


def run_tests_from_generated_manifest(
    manifest_path: str,
    base_dir: str,
    ...
    Reads the manifest to discover tests and their metadata, then runs
    them through the given runtime adapter.
    """
    manifest = Manifest.load(manifest_path)
    base = Path(base_dir)
    results = []

    for suite_data in manifest.suites:
        suite_name = suite_data["name"]
        wasi_version = WasiVersion(suite_data.get("wasi_version", "wasm32-wasip1"))
        meta = TestSuiteMeta(suite_name, wasi_version, runtime.get_meta())
        ...
        for test_entry in suite_data.get("tests", []):
            ...
            wasm_path = str(base / test_entry["wasm"])

            if not os.path.isfile(wasm_path):
                logging.warning("Skipping %s: wasm file not found at %s", name, wasm_path)
                test_cases.append(_skip_single_test(name, Config()))
                for reporter in reporters:
                    reporter.report_test(meta, test_cases[-1])
                continue

            config = _load_config_from_manifest(test_entry, base)
            ...
            for filt in filters:
                ...
                test_case = _execute_single_test(
                    runtime, meta, wasm_path, name, config)
        ...
        for reporter in reporters:
            reporter.report_test_suite(suite)
